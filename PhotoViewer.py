...
class PhotoViewer(QtWidgets.QGraphicsView):
    ...
    def __init__(self, parent):
        ...
        # for accessing different tools from outside, should act like a switch for different drag behaviours
        self._tool = "drag"
        # for storing current press point
        self._pressPoint = QtCore.QPoint(0, 0)
    ...
    def toggleDragMode(self):
        ...
        if self._photo.pixmap().isNull():
            ...
            self.setDragMode(QtWidgets.QGraphicsView.NoDrag)
            self._polygon_item.show()

        elif not self._photo.pixmap().isNull():

            if self._tool == "drag":
                self.setDragMode(QtWidgets.QGraphicsView.ScrollHandDrag)
                self._polygon_item.hide()
    ...
    # defines the mouse press event if the scene is clicked
    def mousePressEvent(self, event):
        if self._photo.isUnderMouse():
            if self._modifiable and not self._tool == "roi":
                self._pressPoint = self.mapToScene(
                    event.pos()).toPoint()  # store press point
                # emit signal which triggers action in main app
                self.photoClicked.emit(self._pressPoint)

            elif self._modifiable and self._tool == "roi" and self._roimode == "active":
                self._polygon_item.removeLastPoint()
                self._polygon_item.addPoint(
                    self.mapToScene(event.pos()).toPoint())
                # movable element
                self._polygon_item.addPoint(
                    self.mapToScene(event.pos()).toPoint())
        # ad mousePressEvent to modified GraphicsView class (Photoviewer)
        super(PhotoViewer, self).mousePressEvent(event)

    # defines the mouse release event if the scene is clicked
    def mouseReleaseEvent(self, event):
        if self._photo.isUnderMouse():
            if self._modifiable and not self._tool == "roi":
                self._releasePoint = self.mapToScene(
                    event.pos()).toPoint()  # store release point
                # emit signal which triggers action in main app
                self.photoClickedReleased.emit(
                    self._pressPoint, self._releasePoint)
    ...
    def mouseMoveEvent(self, event):
        if self._photo.isUnderMouse():
            if self._modifiable and self._tool == "roi" and self._roimode == "active":
                self._polygon_item.movePoint(
                    self._polygon_item.number_of_points()-1, self.mapToScene(event.pos()).toPoint())
